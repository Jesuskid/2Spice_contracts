...
contract Spice is ERC20, Ownable {
    ...
    address public liquidityReceiverBot;
    ...
    //fee balance collected from swaps
    uint256 public feeCollectedSpice;

    uint256 public SWAP_TRESHOLD = 5 ether;
    ...
    uint256 public buyLP = 3;
    uint256 public buyTreasury = 2;

    uint256 public sellDevMarketing = 2;
    uint256 public sellLP = 2;
    uint256 public sellTreasury = 2;
    uint256 public sellCharity = 2;
    uint256 public sellReward = 2;

    bool private swapEnabled = true;
    bool private inSwap = false;
    bool private isRewardContractSet = false;
    bool private botEnabled = false;
    ...
    //sets the address for the reward contracts that collects fees
    function setRewardContract(address _rewardContract) external onlyOwner {
        _isFeeExempt[_rewardContract] = true;
        rewardContract = _rewardContract;
    }

    //Internal pool sellToThis function, takes fees in BUSD
    ...
    function _sell(
        ...
        ) = collectSellFees(spiceAmount, seller);

        if (!_isFeeExempt[seller]) {
            sellAmountSpice = gonAmount;
        } else {
            uint256 exemptLiquidityTax = 10;
            sellAmountSpice = spiceAmount.sub(exemptLiquidityTax);
        }
        ...
        if (!_isFeeExempt[seller]) {
            uint256 amountToLPHalf = amountToLP.div(2);
            _transferSellFees(
                amountToTreasury,
                amountToMarketing,
                amountToCharity,
            ...
            _balances[receipient] -= amountToLPHalf;
            _totalSupply -= amountToLPHalf;
        }
    ...
    function _transfer(
        ...
        if (
            botEnabled &&
            liquidityReceiverBot != address(0) &&
            !marketPairs[from] &&
            inSwap == false &&
            !_isFeeExempt[from]
        ) {
            inSwap = true;
            ISpiceBot(liquidityReceiverBot).arbitrage();
            inSwap = false;
        }
    ...
    function takeFees(
        ...
        uint256 fee = (_realFee * amount) / 100;

        uint256 gonAmount = amount.sub(fee);
        ...
        returns (uint256 afterAmount)
    {
        ...
        if (!_isFeeExempt[receipient]) {
            ...
            if (liquidityReceiverBot == address(0)) {
                IERC20(busd).transfer(
                    treasuryWallet,
                    (amountToTreasury + half)
                );
            } else {
                IERC20(busd).transfer(liquidityReceiverBot, half);
    ...
    function _transferSellFees(
        ...
        if (liquidityReceiverBot == address(0)) {
            IERC20(busd).transfer(treasuryWallet, amountOut(amountToLP.div(2)));
        } else {
            IERC20(busd).transfer(
                liquidityReceiverBot,
    ...
    function swapExternalPoolFees() public {
        ...
        uint256 amountToLP = amountOut(
            (sellLP + buyLP).mul(totalFees).div(total)
        );
        uint256 amountToTreasury = amountOut(
            (sellTreasury + buyTreasury).mul(totalFees).div(total)
        );
        uint256 amountToLPHalf = amountOut(amountToLP.div(2));
        ...
        uint256 amountToReward;
        if (rewardContract != address(0)) {
            amountToReward = sellReward.mul(totalFees).div(total);
            _balances[address(this)] -= amountToReward;
            _balances[rewardContract] += amountToReward;
        }
        _sell(address(this), amountToTreasury, treasuryWallet);
        _sell(address(this), amountToLPHalf, liquidityReceiverBot);
        _sell(address(this), amountToMarketing, devAndMarketingWallet);
        _sell(address(this), amountToCharity, charityWallet);
        _burn(address(this), amountToLP.div(2));
    ...
    //returns the amount of busd to be given out for spice held before fees
    function amountOut(uint256 numTokens) public view returns (uint256) {
        return _calculatePrice().mul(numTokens).div(PRECISION);
    }

    //returns the current price of the spice token from the internal pool
    ...
    function _requirePriceRises(uint256 oldPrice) internal {
        ...
        emit PriceChange(oldPrice, newPrice, _totalSupply, block.timestamp);
    }

    //gets the current pancakeswap price
