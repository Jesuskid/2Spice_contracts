...
contract XUSD is IXSurge, Ownable {
    ...
    constructor(address PromiseUSD, address NewTokenProposal) {
        require(PromiseUSD != address(0) && NewTokenProposal != address(0));
    ...
    function _transferFrom(
        ...
        // fee exempt
        bool isExempt = isTransferFeeExempt[sender] ||
            isTransferFeeExempt[recipient];
        // amount to give recipient
        uint256 tAmount = isExempt
            ? amount
     ...
     */
    function mintWithNative(address recipient, uint256 minOut)
        external
        payable
        override
        returns (uint256)
    ...
    */
    function mintWithBacking(address backingToken, uint256 numTokens)
        external
        nonReentrant
        returns (uint256)
    {
    ...
    */
    ...
    /** 
        ...
        @param tokenAmount Number of XUSD Tokens To Redeem, Must be greater than 0
    */
    function sell(uint256 tokenAmount, address desiredToken)
        external
        notEntered
        returns (address, uint256)
    {
    ...
    */
    function sell(
        ...
        address recipient
    ) external notEntered returns (address, uint256) {
    ...
    function exchange(
        ...
        require(!stableAssets[tokenIn].mintDisabled, "TokenIn Disabled");
        require(
        ...
        require(msg.sender == xSwapRouter, "Only Router");
    ...
    */
     ...
     */
    function requestPromiseTokens(address stable, uint256 amount)
        external
        override
        nonReentrant
        returns (uint256)
    {
        ...
        require(
            amountUnderlyingToDeliver > 0 &&
                amountUnderlyingToDeliver <=
                IERC20(stable).balanceOf(address(this)),
            "Invalid Amount"
        );

        // send Tokens to Seller
        bool successful = IERC20(stable).transfer(
            PROMISE_USD,
            amountUnderlyingToDeliver
        );
    ...
    function requestFlashLoan(
        ...
        require(stableAssets[stable].isApproved, "Not Approved");
        require(
        ...
        require(
            newLiquidity >= oldLiquidity && calculateBacking() >= oldBacking,
        ...
        emit FlashLoaned(
            ...
            stableToRepay,
            amount,
            newLiquidity - oldLiquidity + amount
        );
        return true;
    }
    ...
    function _mintWithNative(address recipient, uint256 minOut)
        internal
        nonReentrant
        returns (uint256)
    {
    ...
    function _mintWithBacking(
        ...
        require(
            stableAssets[token].isApproved && token != address(0),
            "Token Not Approved"
        );
        // users token balance
        ...
        require(
            !stableAssets[token].mintDisabled,
            "Mint Is Disabled With This Token"
        );
    ...
    function _sell(
        ...
        require(
            _validToSend(desiredToken, amountUnderlyingAsset),
            "Invalid Token"
        );
        ...
        // allocate resources
        if (
            shouldAllocateResources(
                isTransferFeeExempt[seller],
                seller,
                recipient
            )
        ) {
        ...
        // send Tokens to Seller
        bool successful = IERC20(desiredToken).transfer(
            recipient,
    ...
    function _swapStables(
        ...
        require(received <= tokenInAmount && received > 0, "SC");
        require(
    ...
    function _transferIn(address token, uint256 desiredAmount)
        internal
        returns (uint256)
    {
    ...
    function getValueOfHoldings(address holder)
        public
        view
        override
        returns (uint256)
    ...
    function isUnderlyingAsset(address token)
        external
        view
        override
        returns (bool)
    ...
    function calculateBacking() public view returns (uint256) {
        uint256 total = liquidityInStableCoinSwap();
    ...
    function expectedTokenToReceive(uint256 amount)
        public
        view
        returns (address)
    {
        for (uint256 i = stables.length - 1; i >= 0; i--) {
    ...
    function shouldAllocateResources(
        bool feeExempt,
        address caller,
        address recipient
    ...
    function _validToSend(address stable, uint256 amount)
        internal
        view
        returns (bool)
    {
    ...
    function upgradeFlashLoanProvider(address flashLoanProvider_)
        external
        onlyOwner
    {
    ...
    function upgradeTokenFetcher(ITokenFetcher tokenFetcher)
        external
        onlyOwner
    {
    ...
    function upgradeResourceCollector(
    ...
    function disableMintForStable(address stable, bool isDisabled)
        external
        onlyOwner
    {
    ...
    */
    function removeStable(address stable, address stableToSwapWith)
        external
        nonReentrant
        onlyOwner
    {
        ...
        require(
            stableToSwapWith != PROMISE_USD && stable != PROMISE_USD,
            "Promise"
        );
        ...
        stableAssets[stables[stables.length - 1]].index = stableAssets[stable]
            .index;

        // replace removed element with last element
        stables[stableAssets[stable].index] = stables[stables.length - 1];
     ...
     */
    function redeemForLostAccount(address account, uint256 amount)
        external
        onlyOwner
    {
    ...
    */
    ...
    function setPermissions(address Contract, bool transferFeeExempt)
        external
        onlyOwner
    {
    ...
    function setApprovedPromiseUSDContract(
        address Contract,
        bool isApprovedForPromiseUSD
    ) external onlyOwner {
        require(Contract != address(0));
        IPromiseUSD(PROMISE_USD).setApprovedContract(
            Contract,
            isApprovedForPromiseUSD
        );
    }
    ...
    event TokenActivated(uint256 blockNo);
    ...
    event SetFees(uint256 mintFee, uint256 transferFee, uint256 sellFee);
}
